		...
		>
			...
			>
				...
				<div className="flex flex-wrap gap-2 items-center mt-6">
					{experience.tags.map((name) => (
						<span
							className="tag border border-green-100 bg-green-200"
							key={name}
						>
							{name}
						</span>
...
const Experience = () => {
	const [animationName, setAnimationName] =
		React.useState("idle");
	const [isPointerOver, setIsPointerOver] =
	...
	return (
		<>
			<div
				className="sm:flex gap-1 mt-20 relative"
				ref={containerRef}
			>
				...
				{isPointerOver && (
					<FlashLight containerRef={containerRef} />
				)}
			</div>
		</>
	);
